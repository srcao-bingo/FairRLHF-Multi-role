...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Whether or not to use reward difference as a regular term for reward signal.",
    )
    parser.add_argument(
        "--coef_lambda",
        type=float,
        default=0.0,
        help="Regularization coefficient of reward difference, default 0.0 .",
    )
...
def evaluation_by_ppl(trainer, eval_dataloader, device):
    ...
    losses = torch.tensor(losses / (count + 1))
    try:
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            if exp_dataset is not None:
                ...
                for exp_data in exp_dataset:
                    reward_score.append(exp_data["rewards"])
                
                print("len(reward_score) = ", len(reward_score))
        
                if args.using_reward_difference == True and len(reward_score) % 2 == 0:
                    coef_lambda = args.coef_lambda
            
                for i in range(0,len(reward_score)-1,2):
                    q_0 = reward_score[i]
                    q_1 = reward_score[i+1]
                    diff = abs(q_0 - q_1)
                    reward_score[i] = reward_score[i] - coef_lambda * diff
                    reward_score[i+1] = reward_score[i+1] - coef_lambda * diff
                
                for i,exp_data in enumerate(exp_dataset):
                    exp_data["rewards"] = reward_score[i]

                    print(type(reward_score))
