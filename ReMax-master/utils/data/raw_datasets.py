...
class PromptRawDataset(object):
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        if "local/jsonfile" not in dataset_name:
...
# AgeBias English datasets
class LocalAgeBiasJsonFileDataset(PromptRawDataset):
    def __init__(self, output_path, seed, local_rank, dataset_name, train_path,eval_path):
        super().__init__(output_path, seed, local_rank, "local/jsonfile/"+dataset_name)
        self.dataset_name = "local/age_bias_json_file"
        self.dataset_name_clean = "age_bias_json_file"
    ...
    def get_prompt_and_chosen(self, sample):
        if sample["instruction"] is not None and sample["output"] is not None:
            return " " + sample["instruction"] + sample["input"] + sample["output"][0]
        return None

    def get_prompt_and_rejected(self, sample):
        if sample["instruction"] is not None and sample["output"] is not None:
            return " " + sample["instruction"] + sample["input"] + sample["output"][1]
        return None
